...
void Game::handleEventPlaying(SDL_Event& e, const Music& music, int& mode, bool& quit)
{
    ...
    if (e.type == SDL_KEYDOWN){
        ...
        if (bridge->checkDir(bridge)) Mix_PlayChannel(-1, music.moveBridge, 0);
    }
...
void Game::start(SDL_Renderer* renderer, bool& quit)
{
...
void Game::help(SDL_Renderer* renderer, bool& quit)
{
    if (lastMode == 1) start(renderer,quit);
    else if (lastMode == 2) playing(renderer,quit);
